...
class MarketConversionServiceTest {
    ...
    private fun getInSelection(name: String, odds: Double): InSelection {
        ...
        inSelection.name = name
        inSelection.odds = odds.toString()
        return inSelection
    }
    ...
    fun testOneXTwo() {
        ...
        assertAll(
            ...
            { assertEquals("e1_1", outMarket.marketUid, "Market UID should be correctly formatted. Expected 'e1_1'") },
    ...
    fun testOneXTwoInsensitiveCaseAndSpaces() {
        ...
        assertAll(
            { assertEquals("1", outMarket.marketTypeId, "Market type ID should be '1' for 1X2 market") },
            { assertEquals("e5_1", outMarket.marketUid, "Market UID should be correctly formatted. Expected 'e5_1'") },
            { assertEquals(3, outMarket.selections?.size, "There should be 3 selections in the output market") },
            { assertEquals(1, outMarket.selections?.get(0)?.selectionTypeId, "Selection type ID for Team A should be 1") },
            { assertEquals(2, outMarket.selections?.get(1)?.selectionTypeId, "Selection type ID for Draw should be 2") },
            { assertEquals(3, outMarket.selections?.get(2)?.selectionTypeId, "Selection type ID for Team B should be 3") }
        )
    }
    ...
    fun testSpecifierWithLeadingSign() {
        ...
        assertAll(
            ...
            { assertEquals(1714, outMarket.selections?.get(0)?.selectionTypeId, "Selection type ID for Team A should be 1714") },
            { assertEquals(1715, outMarket.selections?.get(1)?.selectionTypeId, "Selection type ID for Team B should be 1715") }
        )
    }
    ...
    fun testFirstAndSecondHalfHandicap() {
        val inMarket1 = InMarket("1st half - handicap", "e13", mutableListOf(
            getInSelection("Team A +0.5", 1.9),
            getInSelection("Team B -0.5", 1.9)
        ))

        val inMarket2 = InMarket("2nd half - handicap", "e14", mutableListOf(
        ...
        assertAll(
            { assertEquals("66", outMarket1.marketTypeId, "Market type ID should be '66' for 1st half Handicap market") },
            { assertEquals("e13_66_0.5", outMarket1.marketUid, "Market UID should be correctly formatted. Expected 'e13_66_0.5'") },
            { assertEquals("88", outMarket2.marketTypeId, "Market type ID should be '88' for 2nd half Handicap market") },
            { assertEquals("e14_88_1", outMarket2.marketUid, "Market UID should be correctly formatted. Expected 'e14_88_1'") }
        )
    }
